...
ground_truth=imread('white_matter.tif');
%Superimposed brain and white matter images
figure(1),imshow(im),title('Superimposing of brain and white matter (represented in blue)');
hold on
wm=label2rgb(ground_truth,'jet','w','shuffle');
result=imshow(wm);
set(result, 'AlphaData', 0.3);
hold off 
...
dist1=(im-m1).^2;
figure(2); plotim(dist1);

dist2=(im-m2).^2;
figure(3); plotim(dist2);

dist3=(im-m3).^2;
figure(4); plotim(dist3);


etiq_m = (dist1<=dist2 & dist1<dist3)*1 + ...
          (dist2<=dist3 & dist2<dist1)*2 + ...
          (dist3<=dist1 & dist3<dist2)*3  ;
figure(5) ; imagesc(etiq_m) ; colormap(cool) ; pause(0.5) ;

%Compute the Dice Similarity Coefficient to measure the agreement between the manual
%and automatic segmentation of the white matter
white_matter_index=;
[s1 s2]=size(im);
seg=zeros(s1,s2);
seg(etiq_m==white_matter_index)=1;
dice=2*nnz(seg&ground_truth)/(nnz(seg)+nnz(ground_truth))
...
figure(6) ; imagesc(etiq_km) ; colormap(cool) ; pause(0.5) ;

   %updating of the class centers%Dice Similarity coefficient for the white matter segmentation
dice=2*nnz(Seg&ground_truth)/(nnz(Seg)+nnz(ground_truth))
   m_ = m ;
...
white_matter_index=;
seg=zeros(s1,s2);
seg(etiq_km==white_matter_index)=1;
dice=2*nnz(seg&ground_truth)/(nnz(seg)+nnz(ground_truth))
...
%Dice Similarity coefficient for the white matter segmentation
dice=2*nnz(Seg&ground_truth)/(nnz(Seg)+nnz(ground_truth))